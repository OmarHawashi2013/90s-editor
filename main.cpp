...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE, LPSTR, int nCmdShow){
	...
	*/
	...
	AddFontResourceEx("W95F.otf", FR_PRIVATE, 0);
	...
	*/
	...
	while (true){
		...
		if (GetAsyncKeyState(VK_CONTROL) & 0x8000 && GetAsyncKeyState('S') & 0x8000) {
		    ...
		    if (GetSaveFileNameW(&ofn)) {
		        ...
		        if (hFile != INVALID_HANDLE_VALUE) {
		            ...
		            std::string content = v.to_string(); 
		            
		            unsigned char bom[] = {0xEF, 0xBB, 0xBF};
		            WriteFile(hFile, bom, 3, &dwBytesWritten, NULL);

		            WriteFile(hFile, content.c_str(), (DWORD)content.length(), &dwBytesWritten, NULL);
		...
		if (GetForegroundWindow() == hwnd and !(GetAsyncKeyState(VK_CONTROL) & 0x8000 && GetAsyncKeyState('S') & 0x8000)) {
			for (int i = 0; i < 255; i++) {
